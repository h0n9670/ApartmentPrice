...
# 중학교 주소 데이터
sub <- read.csv("..\\Data\\preprocessingData\\Y_middleschool.csv", stringsAsFactors = F)
...
dist_from_middleschool <- round(distance/1000,2)
View(dist_from_univ)

# merge 데이터에 거리 데이터 병합
df <- cbind(merge, dist_from_middleschool)
...
write.csv(df,"..\\Data\\preprocessingData\\O_20200422_dist_from_middleschool.csv",row.names = FALSE)
