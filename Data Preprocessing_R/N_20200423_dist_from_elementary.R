...
apt <- unique(apt2)
# 초등학교 주소 데이터
sub <- read.csv("Y_elementary.csv", stringsAsFactors = F)
distance <- c()
univer <- c()
for (i in 1:nrow(apt)){
  d <- 100000
...
df1 <- rename(df1, elementaryschool = univer)
df1 <- rename(df1, dist_from_elementaryschool  = distance)
